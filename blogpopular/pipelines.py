...
class BlogpopularPipeline:

    # Database setup
    conn = sqlite3.connect('blogpopular.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `blogpopular`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM blogpopular WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO blogpopular (date, title, link, content)"
